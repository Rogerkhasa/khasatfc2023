...
<body>
   ...
   <h3>TABLEAU DE BORD</h3>
                ...
                <a href="#president" onclik="afficher(){ }"><img src="img/candidat.png" alt="">candidats presidentiels</a>
   ...
   <!-- president -->
   <section class="tout" id="president">
   <?php
include("bdd_conn.php");
...
?>
 <div class="container">
   ...
   <section class="tout" id="national">
...
if((isset($_FILES['image'])) AND (!empty($_FILES['image'])) AND (!empty($_POST['nom'])) 
 AND (!empty($_POST['postnom'])  AND (!empty($_POST['prenom'])))  AND (!empty($_POST['sexe']))){
    ...
    if(empty($errors)==true){
       ...
       $savephoto = $bdd->prepare("INSERT INTO depute_national (nom,postnom,prenom,sexe,voies,photo) VALUES (?,?,?,?,?,?)");
       $savephoto->execute(array($nom, $postnom,$prenom, $sexe,$voies,$lien_photo));
       $ok= "Success";
    }else{
      $errors;
