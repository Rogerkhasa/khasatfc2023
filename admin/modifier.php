...
$id = $_GET['id'];
$selection_pre = $bdd->query("SELECT * FROM presidentielle WHERE id='$id'");
...
if((isset($_FILES['image'])) AND (!empty($_FILES['image'])) AND (!empty($_POST['nom'])) 
 AND (!empty($_POST['postnom'])  AND (!empty($_POST['prenom'])))  AND (!empty($_POST['sexe']))){
    ...
    if(empty($errors)==true){
       ...
       $savephoto = $bdd->query("UPDATE  presidentielle SET id=$id, nom ='$nom' ,postnom='$postnom',prenom='$prenom',sexe='$sexe',photo='$lien_photo' WHERE id=$id ");
...
<div class="container">
         ...
         <marquee behavior="" direction="left"><center><h3>MODIFICATION DU CANDIDAT PRESIDENTIEL</h3></center></marquee>
                <div class="row">
                  <?php if (isset($error)) { ?>
